...
export function renderDiagram(container, room, tableType, result, previewObstacle) {
  ...
  if (result && tableType) {
    ...
    const clearance = tableType.clearanceBuffer;
    const maxBufferIn = tableType.shape === 'round' ? clearance : Math.max(clearance.width, clearance.depth);

    if (maxBufferIn > 0) {
      const stdDev = Math.max(0.5, (maxBufferIn * scale) / 2.5);
      ...
      tableShadows = result.tables.map((t) => tableShadowMarkup(t, tableType, clearance, scale)).join('');
    }
...
function tableShadowMarkup(table, tableType, clearance, scale) {
  const x = table.x * scale;
  const y = table.y * scale;

  if (tableType.shape === 'round') {
    const bufferPx = clearance * scale;
  ...
  // Rectangular tables can have a different clearance per axis (no one sits
  // at the short ends), so the halo isn't uniform - it's wider on the long
  // seating edges (depth axis) than at the ends (width axis).
  const widthBufferPx = clearance.width * scale;
  const depthBufferPx = clearance.depth * scale;
  const w = tableType.dimensions.width * scale + widthBufferPx * 2;
  const h = tableType.dimensions.depth * scale + depthBufferPx * 2;
  const rPx = Math.min(widthBufferPx, depthBufferPx);
  return `<rect x="${x - w / 2}" y="${y - h / 2}" width="${w}" height="${h}" rx="${rPx}" ry="${rPx}" class="table-shadow" filter="url(#buffer-shadow)" />`;
}
